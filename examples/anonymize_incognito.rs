...
use parallel_k::{
    algorithms::incognito::Incognito,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    unsafe {
        std::env::set_var("POLARS_FMT_MAX_COLS", df.width().to_string());
    }
    ...
    let qis = QuasiIdentifiers::from_column_names(&[
        ...
        (
            "marital-status",
            QIType::Categorical {
                path_to_json_hierarchy: "taxonomies/marital-status.json".into(),
            },
        ),
        (
            "occupation",
            QIType::Categorical {
                path_to_json_hierarchy: "taxonomies/occupation.json".into(),
            },
        ),
        (
            "relationship",
            QIType::Categorical {
                path_to_json_hierarchy: "taxonomies/relationship.json".into(),
            },
        ),
        (
            "race",
            QIType::Categorical {
                path_to_json_hierarchy: "taxonomies/race.json".into(),
            },
        ),
        (
            "gender",
            QIType::Categorical {
                path_to_json_hierarchy: "taxonomies/gender.json".into(),
            },
        ),
    ]);
    ...
    let result_maybe = anonymizer.run(&Incognito::default(), k);
    ...
    match result_maybe {
        Ok(mut result) => {
            ...
            let verified =
                util::verify_k_anonymity(&result.anonymized_dataset.df, &qi_colnames, k as usize);
